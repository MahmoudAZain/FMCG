...
Supabase instance for pricing, placement, transitions and RLS · Playwright for the register →
browse → cart → order journey in Arabic at 360px
...
storefront routes · 11 admin routes · 14 tables · 6 database functions
...
| II | Tenant-of-One Data Isolation | RLS enabled on all 14 tables. Cross-customer access blocked by `USING (profile_id = auth.uid())` with matching `WITH CHECK`. Cost prices live in a separate `product_costs` table with **no grant** to `anon`, `customer` or `staff`. `order_status_history` has no write policy for any role. 20-assertion RLS test matrix in [contracts/rls-policies.md](contracts/rls-policies.md). | **PASS** |
...
| VI | Staff Autonomy Over Master Data | 11 admin routes cover products, categories, brands, promotions, cities and staff. Every field in FR-051 is form-editable. Validation is duplicated into database constraints so a staff mistake cannot corrupt data. | **PASS** |
...
weekly export job (T086), but the business owner should know that a catastrophic data loss on
...
└── contracts/
    ...
    ├── rls-policies.md            # Full policy SQL + 20-assertion verification matrix
...
| **6. Discovery + hardening (US6)** | Search, offers view, pagination, performance, scheduled jobs, backup export | 3 | Arabic and English search return the same product; jobs run |
...
fixed, since they touch disjoint routes. Stage 5 needs both. Stage 6 is deferred deliberately —
category browsing is sufficient for a modest catalog, so search earns its place only after the
revenue path works.
...
|---|---|---|
| **No backups on the Supabase free tier** | Total data loss recoverable only to the last export | Weekly export job (T086); flagged to the business owner as the point where paying $25/month buys point-in-time recovery |
...
| Free-tier pausing after 7 idle days | Site down after a quiet week | 6-hourly Cloudflare Cron keep-alive (T085) |
