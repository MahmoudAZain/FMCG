...
## R17. Reporting: where aggregation runs

**Decision**: All report aggregation happens in **Postgres**, as `STABLE` SQL functions taking a
date range and returning result sets. Next.js renders what they return; the Worker never
aggregates.

**Rationale**: The Cloudflare Workers free tier allows **10 ms CPU per invocation**. Pulling
thousands of order rows into a Worker and summing them there would blow that ceiling and burn
egress against the 5 GB Supabase allowance. Postgres aggregates the same data in single-digit
milliseconds and returns a handful of rows. This is the same reasoning that keeps image
...
budget. A nightly rollup would add a moving part, a staleness window, and a reconciliation
failure mode (SC-017 requires figures to reconcile *exactly*), all to solve a performance
problem that does not exist yet. **Trigger point for revisiting**: dashboard queries exceeding
~800 ms, or the orders table passing ~250,000 rows.

**Alternatives considered**: aggregating in the Worker — rejected on CPU and egress.
Materialized views refreshed on a cron — rejected as premature; reintroduces staleness against
an exactness requirement. An external analytics service — violates Principle V.

---

## R18. Reporting: timezone

**Decision**: Every date bucket is computed as `(timestamp AT TIME ZONE 'Africa/Cairo')::date`.
Timestamps remain stored in UTC; only grouping is localized.

**Rationale**: Egypt runs UTC+2 (UTC+3 during summer time). Grouping by UTC date would push
every order placed after 22:00 local into the next day's figures — so a shop's busiest evening
hours would land on the wrong business day, and daily totals would never match what staff
counted. This is a small implementation detail with a large credibility cost: an owner who
finds the numbers disagree with the till stops trusting the dashboard entirely.

Egypt's summer-time transitions are handled by naming the zone rather than hard-coding an
offset. `Africa/Cairo` is a single constant in the SQL layer, not repeated per query.

---

## R19. Reporting: export formats

**Decision**: **CSV encoded as UTF-8 with a byte-order mark**, plus **XLSX generated in the
browser** from JSON the server returns.

**The BOM is not optional.** Excel on Windows opens a UTF-8 CSV without a BOM using the system
ANSI codepage, which renders Arabic product names as mojibake — `Ù„Ø¨Ù† Ø¬Ù‡ÙŠÙ†Ø©` instead
of `لبن جهينة`. The three-byte BOM (`EF BB BF`) makes Excel read it as UTF-8. For a bilingual
Egyptian business this is the difference between a usable export and a useless one (FR-080,
SC-020).

Fields are quoted, embedded quotes doubled, and CRLF line endings used, per RFC 4180. Values
beginning `=`, `+`, `-` or `@` are prefixed with a single quote to prevent spreadsheet formula
injection — a product name is data, and must never be evaluated when someone opens the file.

**XLSX is generated client-side** by dynamically importing a writer library only on the reports
route. Building XLSX server-side means zipping XML inside a 10 ms CPU budget; doing it in the
browser costs the Worker nothing, keeps the storefront bundle untouched (the import never
reaches customer-facing routes), and gives proper column types, number formats and RTL sheet
direction for the Arabic view.

**Money in exports** is written as decimal EGP (`45.00`), converted from stored piastres at the
serialization boundary — an accountant should not have to divide by 100. Piastres remain the
only representation inside the system (R5).

**Alternatives considered**: server-side XLSX — rejected on the CPU ceiling. CSV without BOM —
rejected; breaks Arabic in the single most likely destination application. PDF reports —
deferred; spreadsheets are what an accountant asks for.

---

## R20. Reporting: cost and margin isolation

**Decision**: Cost, margin and profit live in **separate admin-only report functions**, not as
suppressed columns on shared ones. A staff-visible report has no cost column in its return type
at all.

**Rationale**: Principle II again, and the same reasoning as R8. Filtering a column out in the
presentation layer means one forgotten `select *`, one CSV serializer iterating object keys, or
one future "add all columns to the export" convenience, leaks margin data to whoever can reach
the report. Because exports serialize whatever the query returned, a suppressed-column approach
is *especially* fragile in reporting — the file is generated by iteration, not by a hand-written
list of fields.

Report functions therefore come in pairs where both roles need a view: `report_product_sales()`
for staff (units, revenue) and `report_product_margin()` for admins (adds cost, margin, profit),
the latter carrying an `is_admin()` guard that raises rather than returns empty. FR-078 and
SC-018 are then verifiable by inspecting return types, not by auditing every call site.

**Export auditing** (FR-085): reports containing customer contact details or financial totals
write an `admin_audit_log` entry naming the actor, the report, the range and the row count.
Exports leave the system; that is worth a trace.
...
| Where report aggregation runs | Postgres SQL functions; no rollup table in v1 (R17) |
| Report date grouping | `Africa/Cairo` local dates, UTC storage (R18) |
| Export formats | CSV as UTF-8 **with BOM**; XLSX generated in the browser (R19) |
| Margin data in reports | Separate admin-only functions, not suppressed columns (R20) |
